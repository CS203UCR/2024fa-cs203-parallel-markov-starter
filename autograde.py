...
def compute_scores(df, FOM, targets):
    ...
    df = df.loc[(df["function"] == "markov_solution_c")].copy().reset_index()
...
def compute_all_scores(dir=None):
    ...
    bench["label"] = bench["function"] + " " + list(map(str,bench["change_scale"]))+ " " + list(map(str,bench["days"])) 
    #                                     baseline ET     Speedup
...
def autograde(submission=None, results=None):
    try:
        failures = 3 - compute_correctness(dir=submission)
